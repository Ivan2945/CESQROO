...
export async function POST(req: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  const [{ data: riders }, { data: horses }] = await Promise.all([
    supabaseAdmin.from("riders").select("id, first_name, last_name").eq("club_id", clubId).order("last_name"),
    supabaseAdmin.from("horses").select("id, name").eq("club_id", clubId).order("name"),
  ]);
